...
echo "======================================================================="
echo "|                                                                     |"
echo "|    full-stack-proxy-nginx-odoo-for-everyone-with-docker-compose     |"
echo "|                       by Erdal ALTIN                                |"
echo "|                                                                     |"
echo "======================================================================="
sleep 2
...
then
	...
	ssl_snippet="certbot certonly --webroot --webroot-path \/tmp\/acme-challenge --rsa-key-size 4096 --non-interactive --agree-tos --no-eff-email --force-renewal --email \$\{LETSENCRYPT_EMAIL\} -d \$\{DOMAIN_NAME\} -d www.\$\{DOMAIN_NAME\} -d mail.\$\{DOMAIN_NAME\}"
fi
...
if [ -x "$(command -v docker)" ] && [ "$(docker compose version)" ]; then
	...
	# installing Odoo and the other services
	docker compose up -d & export pid=$!
	echo "Odoo and the other services installing proceeding..."
	echo
	wait $pid
	if [ $? -eq 0 ]
	then
		...
		echo
		echo "Error! could not installed Odoo and the other services with docker compose" >&2
		echo
		exit 1
	fi
else
